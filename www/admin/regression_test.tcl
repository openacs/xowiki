...
test subsection "Basic Setup: Package, url= /$instance_name/en/index"
...
? {$package_id set object} en/index "object name parsed"
...
########################################################################
test section "Edit new ::xowiki::PlainPage via weblink"

::xowiki::Package initialize -parameter $index_vuh_parms \
    -package_id $info(package_id) \
    -url /$instance_name/ \
    -actual_query "object%5ftype=%3a%3axowiki%3a%3aPlainPage&edit%2dnew=1" \
    -user_id [lindex $swas 0]

set content [::$package_id invoke -method $m]
? {string first Error $content} -1 "page contains no error"
? {expr {[string first "Plain Page" $content]>-1}} 1 \
    "page contains Plain Page"

regexp {name="item_id" value="([^\"]+)"} $content _ returned_item_id
? {info exists returned_item_id} 1 "item_id contained in form"
? {expr {$returned_item_id > 0}} 1 "item_id $returned_item_id > 0"
? {$package_id isobject $returned_item_id} 0 "item is not instantiated"
...
########################################################################
test section "Submit edited ::xowiki::PlainPage via weblink"

set return_url /$instance_name/admin
...
::xowiki::Package initialize -parameter $index_vuh_parms \
    ...
    -form_parameter [subst {
      ...
      __key_signature {$signature} 
      __object_name {}
      name en:plain_page_1
      object_type ::xowiki::PlainPage
      text.format text/html 
      creator {Gustaf Neumann} 
      description {{this is the description}}
      text {{Content of Plain Page}} 
      nls_language en_US 
      folder_id $returned_folder_id 
      title {{Plain Page 1}}
      item_id $returned_item_id }]
...
set content [test without_ns_form {::$package_id invoke -method $m}]
? {string first Error $content} -1 "page contains no error"
? {expr {[string first "Content of Plain Page" $content]>-1}} 1 \
    "page contains specified content"

? {::xo::cc exists __continuation} 1 "continuation exists"
? {::xo::cc set  __continuation} \
    "ad_returnredirect /$instance_name/en/plain_page_1" \
    "redirect to PlainPage"
